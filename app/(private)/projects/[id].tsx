...
import { ArrowLeft, ChevronLeft, Eye, TreePine, Truck } from 'lucide-react-native';
...
export default function ProjectDetailScreen() {
  ...
  return (
    <View className="flex-1 bg-background-secondary">
      <Header
        headerLeft={
          <Button onPress={() => router.back()} className="w-[40px] h-[40px] flex-row items-center bg-background-primary rounded-full">
            <ChevronLeft size={22} color={colorTheme['textPrimary']} />
          </Button>
        }
        headerCenter={
          <Text className="text-[22px] font-semibold text-white font-open-sans-bold">EZ Debris</Text>
        }
      />
