...
const Createticket = () => {
  ...
  const { schema } = useFieldMonitorSchemaStore();
  const { setValue } = useFieldMonitorStore();
  ...
  const requestLocation = async () => {
    ...
    setValue("lat_long", loc.coords.latitude + "," + loc.coords.longitude);
    setValue("address", addresses[0]?.formattedAddress ?? "");
